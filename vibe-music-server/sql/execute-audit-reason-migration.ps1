# SQL 迁移文件执行脚本
# 用途：执行 add_audit_reason_field.sql 迁移文件，为相关表添加审核备注字段
...
param(
    ...
    [string]$SqlFile = "add_audit_reason_field.sql"
)
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "已为以下表添加 audit_reason 字段:" -ForegroundColor Cyan
        Write-Host "  - tb_song" -ForegroundColor White
        Write-Host "  - tb_forum_post" -ForegroundColor White
        Write-Host "  - tb_forum_reply" -ForegroundColor White
        Write-Host "  - tb_comment" -ForegroundColor White
        Write-Host ""
