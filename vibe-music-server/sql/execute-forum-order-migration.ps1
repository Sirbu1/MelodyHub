# SQL 迁移文件执行脚本
# 用途：执行 create_forum_order_table.sql 迁移文件，创建论坛接单表
...
param(
    ...
    [string]$SqlFile = "create_forum_order_table.sql"
)
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  执行以下 SQL 查询来验证表是否创建成功：" -ForegroundColor White
        Write-Host "  DESCRIBE tb_forum_order;" -ForegroundColor Gray
        Write-Host "  或者：" -ForegroundColor White
        Write-Host "  SHOW TABLES LIKE 'tb_forum_order';" -ForegroundColor Gray
    } else {
        Write-Host ""
