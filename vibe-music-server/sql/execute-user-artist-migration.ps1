# SQL 迁移文件执行脚本
# 用途：执行 add_user_artist_fields.sql 迁移文件，为用户表添加歌手相关字段
...
param(
    ...
    [string]$SqlFile = "add_user_artist_fields.sql"
)
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "  DESCRIBE tb_user;" -ForegroundColor Gray
        Write-Host ""
        Write-Host "  或者执行：" -ForegroundColor White
        Write-Host "  SHOW COLUMNS FROM tb_user LIKE 'gender';" -ForegroundColor Gray
        Write-Host "  SHOW COLUMNS FROM tb_user LIKE 'birth';" -ForegroundColor Gray
        Write-Host "  SHOW COLUMNS FROM tb_user LIKE 'area';" -ForegroundColor Gray
    } else {
        Write-Host ""
