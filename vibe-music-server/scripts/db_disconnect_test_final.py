...
测试流程：
1. 100用户进行收藏、评论操作（持续进行）
2. 在测试过程中手动断开数据库连接（执行: net stop MySQL80）
3. 观察用户操作反馈
...
注意：
    - 需要在另一个终端手动执行: net stop MySQL80 和 net start MySQL80
    - 或者使用管理员权限自动执行（需要修改脚本）
"""
...
class Statistics:
    ...
    def add_result(self, result: OperationResult):
        ...
        if result.error_message:
            self.errors.append(f"User {result.user_id}: {result.operation_type} - {result.error_message}")
    ...
    try:
        ...
        async with session.post(url, headers=headers, params=params, timeout=aiohttp.ClientTimeout(total=30)) as response:
            ...
            if response.status == 200:
                ...
                return False, elapsed, f"HTTP {response.status}", is_disconnected, http_status
    except asyncio.TimeoutError:
        elapsed = time.time() - start_time
        return False, elapsed, "请求超时", is_disconnected, 0
    except Exception as e:
        elapsed = time.time() - start_time
        return False, elapsed, f"异常: {str(e)[:50]}", is_disconnected, 0
...
async def add_comment(session: aiohttp.ClientSession, token: str, song_id: int, content: str) -> tuple[bool, float, str, bool, int]:
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {token}"}
        data = {"songId": song_id, "content": content}
...
async def simulate_user_operations(user_id: int, username: str, password: str, email: str, 
                                  song_ids: List[int], end_time: float):
    """模拟单个用户的操作（持续进行直到结束时间）"""
    results = []
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            while time.time() < end_time:
                ...
                operation_type = "collect" if operation_count % 2 == 0 else "comment"
                ...
                if operation_type == "collect":
                    ...
                    content = f"测试评论_{username}_{operation_count}_{int(time.time())}"
...
def check_data_integrity():
    ...
    if disconnect_start_time > 0 and reconnect_time > 0:
        print(f"实际断连时长: {reconnect_time - disconnect_start_time:.2f}秒")
    ...
    try:
        ...
        # 检查收藏数据
        cursor.execute("SELECT COUNT(*) FROM tb_user_favorite WHERE create_time >= DATE_SUB(NOW(), INTERVAL 5 MINUTE)")
        favorite_count = cursor.fetchone()[0]
        print(f"\n最近5分钟新增收藏数: {favorite_count}")
        
        # 检查评论数据
        cursor.execute("SELECT COUNT(*) FROM tb_comment WHERE create_time >= DATE_SUB(NOW(), INTERVAL 5 MINUTE)")
        comment_count = cursor.fetchone()[0]
        print(f"最近5分钟新增评论数: {comment_count}")
        ...
        collect_diff = abs(favorite_count - expected_collects)
        comment_diff = abs(comment_count - expected_comments)
        
        if collect_diff <= 10:
            print(f"\n[OK] 收藏数据完整性检查通过（差异: {collect_diff}）")
        else:
            print(f"\n[WARNING] 收藏数据可能不完整，差异: {collect_diff}")
        
        if comment_diff <= 10:
            print(f"[OK] 评论数据完整性检查通过（差异: {comment_diff}）")
        else:
            print(f"[WARNING] 评论数据可能不完整，差异: {comment_diff}")
        
        # 分析断连期间的操作
        if len(disconnect_ops) > 0:
            print(f"\n断连期间操作分析:")
            print(f"  总操作数: {len(disconnect_ops)}")
            print(f"  成功操作: {len(successful_during_disconnect)}")
            print(f"  失败操作: {len(failed_during_disconnect)}")
            if len(successful_during_disconnect) > 0:
                print(f"  注意: 断连期间有 {len(successful_during_disconnect)} 个操作返回成功")
                print(f"        这些操作可能通过连接池缓存或重试机制完成")
...
async def main():
    """主测试函数"""
    global TEST_SONG_IDS, db_disconnected, disconnect_start_time, reconnect_time, test_start_time
    ...
    print("="*70)
    print("\n重要提示:")
    print("  请在测试开始后约15秒时，在另一个终端执行: net stop MySQL")
    print("  等待约60秒后，执行: net start MySQL")
    print("  脚本会自动检测数据库连接状态\n")
    ...
    for i in range(CONCURRENT_USERS):
        ...
        task = simulate_user_operations(i + 1, user["username"], user["password"], user["email"], TEST_SONG_IDS, end_time)
        tasks.append(task)
    
    # 监控数据库连接状态
    async def monitor_db_status():
        global db_disconnected, disconnect_start_time, reconnect_time
        last_status = True
    ...
    # 启动测试任务
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 开始执行测试，请在15秒后断开数据库连接...\n")
