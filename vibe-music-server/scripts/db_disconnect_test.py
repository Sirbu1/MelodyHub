...
class OperationResult:
    ...
    operation_id: int  # song_id 或 comment_id
    success: bool
    response_time: float
    error_message: str = ""
    timestamp: float = 0
...
def disconnect_database():
    ...
    try:
        print(f"\n[数据库操作] 正在断开数据库连接...")
        # Windows: 停止MySQL服务
        result = subprocess.run(["net", "stop", "MySQL80"], shell=True, capture_output=True, text=True)
        if result.returncode == 0 or "服务名无效" not in result.stderr:
            disconnect_start_time = time.time()
            db_disconnected = True
            print(f"[数据库操作] 数据库连接已断开 (时间: {datetime.now().strftime('%H:%M:%S')})")
            return True
        else:
            print(f"[数据库操作] 无法停止MySQL服务，可能需要管理员权限")
            print(f"[数据库操作] 请手动执行: net stop MySQL80")
            return False
    except Exception as e:
        print(f"[数据库操作] 断开数据库连接失败: {e}")
        print(f"[数据库操作] 请手动停止MySQL服务: net stop MySQL80")
        return False


async def reconnect_database():
    """恢复数据库连接（通过启动MySQL服务）"""
    global db_disconnected, reconnect_time
    try:
        print(f"\n[数据库操作] 正在恢复数据库连接...")
        # Windows: 启动MySQL服务
        result = subprocess.run(["net", "start", "MySQL80"], shell=True, capture_output=True, text=True)
        if result.returncode == 0:
            ...
            db_disconnected = False
            print(f"[数据库操作] 数据库连接已恢复 (时间: {datetime.now().strftime('%H:%M:%S')})")
            if disconnect_start_time > 0:
                print(f"[数据库操作] 断连持续时间: {reconnect_time - disconnect_start_time:.2f}秒")
            return True
        else:
            print(f"[数据库操作] 无法启动MySQL服务，可能需要管理员权限")
            print(f"[数据库操作] 请手动执行: net start MySQL80")
            return False
    except Exception as e:
        print(f"[数据库操作] 恢复数据库连接失败: {e}")
        print(f"[数据库操作] 请手动启动MySQL服务: net start MySQL80")
        return False
...
async def collect_song(session: aiohttp.ClientSession, token: str, song_id: int) -> tuple[bool, float, str, bool]:
    """收藏歌曲"""
    ...
    try:
        ...
        elapsed = time.time() - start_time
        return False, elapsed, f"异常: {str(e)}"


async def add_comment(session: aiohttp.ClientSession, token: str, song_id: int, content: str) -> tuple[bool, float, str, bool]:
    """添加评论"""
    start_time = time.time()
    is_disconnected = db_disconnected
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i in range(operation_count):
                ...
                if operation_type == "collect":
                    success, response_time, error_msg, was_disconnected = await collect_song(session, token, song_id)
                else:
                    content = f"测试评论_{username}_{i}_{int(time.time())}"
                    success, response_time, error_msg, was_disconnected = await add_comment(session, token, song_id, content)
...
def check_data_integrity():
    ...
    try:
        ...
        if duplicate_favorites:
            print(f"\n警告: 发现 {len(duplicate_favorites)} 条重复收藏记录")
        ...
        if duplicate_comments:
            print(f"\n警告: 发现 {len(duplicate_comments)} 条重复评论记录")
        else:
            print(f"[OK] 未发现重复评论记录")
        
        # 统计操作成功率
        expected_collects = len([op for op in stats.operations if op.operation_type == "collect" and op.success])
        expected_comments = len([op for op in stats.operations if op.operation_type == "comment" and op.success])
        ...
        if abs(favorite_count - expected_collects) <= 5:  # 允许5个误差
            print(f"\n[OK] 收藏数据完整性检查通过")
        else:
            print(f"\n[WARNING] 收藏数据可能不完整，差异: {abs(favorite_count - expected_collects)}")
        
        if abs(comment_count - expected_comments) <= 5:
            print(f"[OK] 评论数据完整性检查通过")
        else:
...
async def main():
    ...
    print(f"数据库断连时长: {DB_DISCONNECT_DURATION}秒")
    ...
    # 启动测试任务
    print(f"\n开始执行 {CONCURRENT_USERS} 个用户的操作测试...")
    
    # 启动异步任务
    test_task = asyncio.create_task(asyncio.gather(*tasks))
    
    # 等待10秒后自动断开数据库
    await asyncio.sleep(10)
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 自动断开数据库连接...")
    disconnect_database()
    
    # 等待断连持续时间
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 数据库断连中，持续 {DB_DISCONNECT_DURATION} 秒...")
    await asyncio.sleep(DB_DISCONNECT_DURATION)
    
    # 恢复数据库连接
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 恢复数据库连接...")
    await reconnect_database()
    
    # 等待所有操作完成
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] 等待所有操作完成...")
    all_results = await test_task
