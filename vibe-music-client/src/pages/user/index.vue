...
const refreshUserInfo = async () => {
  ...
  try {
    ...
    if (userInfoResponse.code === 0) {
      console.log('刷新用户信息 - 后端返回的数据:', userInfoResponse.data)
      console.log('刷新用户信息 - 积分字段:', userInfoResponse.data.score)
...
onMounted(async () => {
  if (!userStore.isLoggedIn) {
    ...
    // 如果已登录，先刷新用户信息，然后初始化表单数据
    await refreshUserInfo()
  }
})

// 页面激活时重新初始化表单数据（从其他页面返回时）
onActivated(async () => {
  if (userStore.isLoggedIn) {
    // 页面激活时也刷新用户信息，确保显示最新的积分
    await refreshUserInfo()
  }
})
  ...
  <div class="user-container">
    ...
    <el-form ref="userFormRef" :model="userForm" :rules="userRules" label-width="0" size="large" class="user-form">
      ...
      <div class="section">
        ...
        <el-form-item>
          <el-input :value="(userStore.userInfo.score !== undefined && userStore.userInfo.score !== null) ? userStore.userInfo.score : 100" disabled>
            <template #prepend>
              <span class="text-primary font-semibold">当前积分：</span>
            </template>
            <template #append>
              <el-button size="small" @click="refreshUserInfo" :loading="loading" type="primary" plain>
                刷新
              </el-button>
            </template>
          </el-input>
