...
public class UserFavoriteServiceImpl extends ServiceImpl<UserFavoriteMapper, UserFavorite> implements IUserFavoriteService {
    ...
    public Result<PageResult<SongVO>> getUserFavoriteSongs(SongDTO songDTO) {
        ...
        Map<Long, SongVO> songMap = new LinkedHashMap<>();
        for (SongVO songVO : songPage.getRecords()) {
            if (songVO.getSongId() != null && !songMap.containsKey(songVO.getSongId())) {
                songVO.setLikeStatus(LikeStatusEnum.LIKE.getId()); // 设置为已收藏
                songMap.put(songVO.getSongId(), songVO);
            }
        }
        List<SongVO> songVOList = new ArrayList<>(songMap.values());

        return Result.success(new PageResult<>((long) songMap.size(), songVOList));
    }
    ...
    public Result cancelCollectSong(Long songId) {
        ...
        QueryWrapper<UserFavorite> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("user_id", userId).eq("type", 0).eq("song_id", songId);
        int deletedCount = userFavoriteMapper.delete(queryWrapper);
